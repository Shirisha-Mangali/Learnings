...
insert into courses values(106,'abcd',4);
...
END;
go
SELECT student_id, marks, dbo.fn_GetGrade(marks) AS Grade
FROM results;
go

--get all the students from a city --inline table valued function
go
...
go
SELECT * FROM dbo.fn_StudentsByCity('Chennai');
...
('AI Chatbot', '2023-01-01', '2023-06-30', 1),
('Cloud Migration', '2023-03-15', '2023-12-31', 2),
('Recruitment Drive', '2023-05-01', '2023-07-31', 3),
('Network Upgrade', '2023-02-01', '2023-09-30', 2),
('HR Analytics', '2023-04-01', '2023-10-31', 3);
...
(1, 1, 120), (2, 2, 90), (3, 3, 80), (4, 1, 150), 
(5, 5, 100), (6, 2, 110), (7, 1, 140), (8, 4, 130),
(9, 5, 95), (10, 3, 105), (11, 2, 100), (12, 4, 85);

-- Salary history (tracks salary changes)
...
--Create a Scalar Function that takes emp_id and returns "Name – Department – Salary".
go
create function dbo.get_Details1(@emp_id int)
returns varchar(50)
as
begin
    declare @res varchar(50);
     select @res= concat(emp_name,' - ',dept_name,' - ',emp_salary) from Employee e join department d on e.dept_id=d.dept_id
     where emp_id=@emp_id;
     return @res;

end;
go
select emp_id,dbo.get_Details1(emp_id) from Employee ;

--2.Create a Table-Valued Function that returns top N employees by salary in a dept
go
create function dbo.get_topn(@n int)
returns table 
as 
return(
    ;with cte as(
        select d.dept_name,emp_name,emp_salary,ROW_NUMBER() over(partition by d.dept_id order by emp_salary desc) as rnk
        from Employee e join department d on
        e.dept_id=d.dept_id
        )
    select dept_name,emp_name,emp_salary from cte where rnk<=@n
);
go

select * from dbo.get_topn(5);
--you cannot use a CTE in an inline table-valued function (ITVF) because an ITVF is essentially a single SELECT statement without procedural logic.
go
drop function if exists dbo.get_topn;
create function dbo.get_topn(@n int)
returns @res table(dept_id int,dept_name varchar(30), emp_name varchar(50),emp_salary decimal)
as 
    begin
    ;with cte as(
        select d.dept_id,d.dept_name,emp_name,emp_salary,ROW_NUMBER() over(partition by e.dept_id order by emp_salary desc) as rnk
        from Employee e join department d on
        e.dept_id=d.dept_id
        )
    insert into @res 
    select dept_id,dept_name,emp_name,emp_salary from cte
    where rnk<=@n;
return;
end;
go
select * from dbo.get_topn(2);

--3.Create a function to return salary growth trend of an employee.
go
create function dbo.getSalaryGrowthTrend(@empid int)
returns table
as
return
(
    select emp_id,salary,effective_date,
    lag(salary) over(partition by emp_id order by effective_date) as prevSal,
    salary-lag(salary) over(partition by emp_id order by effective_date) as growth,
    cast(100.0*(salary-lag(salary) over(partition by emp_id order by effective_date))/
    nullif(lag(salary) over(partition by emp_id order by effective_date),0) as decimal(10,2)) as growthpercent
    from SalariesHistory where emp_id=@empid
);
go
select * from dbo.getSalaryGrowthTrend(3);

--4.Write a function to return total hours worked by an employee across projects.

create function emp_Worked_hrs(@empid int)
returns 
...
CREATE TABLE Student_Audit (
  ...
  OldAge INT NULL,
  NewAge INT NULL,
  ActionTime DATETIME DEFAULT SYSUTCDATETIME(),
  ActionBy SYSNAME DEFAULT SUSER_SNAME()
);
...
as
begin
    insert into Student_Audit(StudentID,ActionType,NewAge)
    select StudentID,'INSERT',Age from inserted;
end;

insert into Student(StudentName,Age) values('xyz',20),('abcgd',22);
select * from Student;
select * from Student_Audit;

--AFTER UPDATE trigger (capture old + new)
go
...
as
begin
insert into dbo.Student_Audit(StudentID,ActionType,OldAge,NewAge)
select d.StudentID,'UPDATE',d.Age,i.Age
from deleted d join inserted i on d.StudentID=i.StudentID;
end;

update Student set Age=24 where StudentID=1;
...
as
begin
update dbo.Student_Audit set ActionType='DELETE' where StudentID=1;
end;
insert into Student(StudentName,Age) values('xydtrs',20),('fyfd',22);


--instead of
drop trigger trg_Student_NoDelete;
go
CREATE TRIGGER trg_Student_NoDelete
ON Student
INSTEAD OF DELETE
AS
BEGIN
    PRINT 'Direct delete is not allowed. Use a proper archive process!';
END;

DELETE FROM Student WHERE StudentID = 1;

--Redirect Delete → Audit Table
go
...
AS
BEGIN
    INSERT INTO Student_Audit (StudentID, ActionType, OldAge, NewAge)
    SELECT d.StudentID, 'DELETE', d.Age, NULL
    FROM deleted d;

    -- Optionally, delete from Student
    DELETE s
    FROM Student s
...
--unupdatble view -complex
go
CREATE VIEW vw_StudentWithAudit
AS
SELECT 
    s.StudentID,
    s.StudentName,
    ...
    a.ActionBy
FROM Student s
LEFT JOIN Student_Audit a
    ON s.StudentID = a.StudentID;



UPDATE vw_StudentWithAudit
SET Age = 30
WHERE StudentID = 2;

go
CREATE TRIGGER trg_vw_StudentWithAudit_Update
ON vw_StudentWithAudit
INSTEAD OF UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    UPDATE s
    SET s.StudentName = i.StudentName,
        s.Age = i.Age
    FROM Student s
...
AS
BEGIN
    PRINT 'Table drop attempt detected!';
    ROLLBACK; -- stop the DROP from happening
END;

DROP TABLE Student;

--Log Schema Changes into an Audit Table
CREATE TABLE DDL_AuditLog (
    AuditID INT IDENTITY PRIMARY KEY,
    EventType NVARCHAR(100),
    ObjectName NVARCHAR(200),
    EventTime DATETIME DEFAULT SYSUTCDATETIME(),
...
AS
BEGIN
    INSERT INTO DDL_AuditLog (EventType, ObjectName, LoginName)
    SELECT 
